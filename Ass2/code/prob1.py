...
def FindHomography(Matches, BaseImage_kp, SecImage_kp):
    ...
    for i,Match in enumerate(Matches):
        ...
        #BaseImage_pts.append(BaseImage_kp[Match[0]])
        #SecImage_pts.append(SecImage_kp[Match[1]])
...
def warp_perspective(src_image, homography_matrix, output_size):
    output_image = np.zeros((output_size[1], output_size[0], src_image.shape[2]), dtype=src_image.dtype)
    
    for y in range(output_size[1]):
        for x in range(output_size[0]):
            transformed_point = np.dot(homography_matrix, np.array([x, y, 1]))
            transformed_point /= transformed_point[2]
            fx, fy = transformed_point[0], transformed_point[1]
            
            if 0 <= fx < src_image.shape[1] - 1 and 0 <= fy < src_image.shape[0] - 1:
                px, py = int(fx), int(fy)
                dx, dy = fx - px, fy - py
                
                # Bilinear interpolation
                tl = src_image[py, px] * (1 - dx) * (1 - dy)
                tr = src_image[py, px + 1] * dx * (1 - dy)
                bl = src_image[py + 1, px] * (1 - dx) * dy
                br = src_image[py + 1, px + 1] * dx * dy
                
                output_image[y, x] = tl + tr + bl + br
            else:
                output_image[y, x] = [0, 0, 0]  # Handle out-of-bounds pixels
...
def StitchImages(BaseImage, SecImage):
    ...
    #Matches, BaseImage_kp, SecImage_kp = sift_try.pb1(BaseImage, SecImage_Cyl)
    
    #hm = FindHomography(Matches, BaseImage_kp, SecImage_kp)
    hm = sift_try.pb1(BaseImage, SecImage_Cyl)
    ...
    #SecImage_Transformed = warp_perspective(SecImage_Cyl, hm, (NewFrameSize[1], NewFrameSize[0]))
    SecImage_Transformed_Mask = cv2.warpPerspective(SecImage_Mask, hm, (NewFrameSize[1], NewFrameSize[0]))
    #SecImage_Transformed_Mask = warp_perspective(SecImage_Mask, hm, (NewFrameSize[1], NewFrameSize[0]))
...
if __name__ == "__main__":
    prob1('Images/Field/','Images/Field/f.png')