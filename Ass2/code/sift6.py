...
def get_matches(img_a, img_b, num_keypoints=1000, threshold=0.8):
    ...
    kp_a, desc_a = computeKeypointsAndDescriptors(img_a_gray)
    print("Part1_done")
    kp_b, desc_b = computeKeypointsAndDescriptors(img_b_gray)
    print("Part2_done")
    
    bf = cv2.BFMatcher()
    matches_list = bf.knnMatch(desc_a, desc_b, k=2)
...
def draw_matches(img_a, img_b, kp_a, kp_b, matches):
    # Draw matches between two images
    img_matches = cv2.drawMatches(img_a, kp_a, img_b, kp_b, matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    # Display the matches
...
# # Load the two images
#     image_path_a = "Images/Office/1.jpg"  # Replace with the path to your first image
#     image_path_b = "Images/Office/2.jpg"  # Replace with the path to your second image

#     img_a = cv2.imread(image_path_a)
#     img_b = cv2.imread(image_path_b)

#     img_a = cv2.resize(img_a, (img_a.shape[1]//8, img_a.shape[0]//8))
#     img_b = cv2.resize(img_b, (img_b.shape[1]//8, img_b.shape[0]//8))

#     # Image1 = cv2.imread("Images/Field/1.jpg")
#     # Image2 = cv2.imread("Images/Field/2.jpg")

#     # Get matching keypoints between the two images
#     kp_a, kp_b = get_matches(img_a, img_b)

#     # Convert keypoints to OpenCV's KeyPoint format
#     kp_a_cv2 = [cv2.KeyPoint(pt[0], pt[1], 1) for pt in kp_a]
#     kp_b_cv2 = [cv2.KeyPoint(pt[0], pt[1], 1) for pt in kp_b]

#     # Find matches between keypoints
#     matches = [cv2.DMatch(i, i, 0) for i in range(len(kp_a))]

#     # Draw matches between the two images
#     draw_matches(img_a, img_b, kp_a_cv2, kp_b_cv2, matches)