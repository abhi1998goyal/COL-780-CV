...
def SIFT(img):
    ...
    return key_pts, np.array(descriptors)
...
def refine_keypoints(candidates, octaves, num_keypoints_per_patch=2):
    keypoints = []
    for i in range(len(octaves)):
        ...
        for j,candidate in enumerate(candidates[i]):
            if j==0 or j == len(candidates[i])-1:
                continue 
            scale = j
            #scores = [(x, y, scale, octaves[i][scale][y, x]) for x, y in candidate]
            #scores.sort(key=lambda x: -x[3])
           # for cand in scores[:num_keypoints_per_patch]:
    ...
    for octave in octaves:
        ...
        for j,scale in enumerate(octave):
            if(j!=0 and j!=len(octave)-1):
                ...
                #octave_candidates.extend(local_minima)
...
def create_octaves(image, num_octaves=2, num_scales=8, sigma=1.6):
    octaves = []
    k = 2**(1/num_scales)
...
def compute_descriptors(keypoints, octaves):
    ...
    for i,octave_keypoints in enumerate(keypoints):
        for keypoint in octave_keypoints:
            ...
            key_pts.append(((2**(scale))*x,(2**(scale))*y))
...
def draw_matches(img1, keypoints1, img2, keypoints2, matches):
    ...
    for match in matches:
        ...
        (x1, y1) = keypoints1[match[0]]
        (x2, y2) = keypoints2[match[1]]
...
def main():
    ...
    img1 = cv2.imread('Images/Office/3.jpg')
    img2 = cv2.imread('Images/Office/4.jpg')
    
    # Preprocess images
    
    img1 = cv2.resize(img1, (img1.shape[1]//16, img1.shape[0]//16))
    img2 = cv2.resize(img2, (img2.shape[1]//16, img2.shape[0]//16))
    ...
    # InitialMatches = BF_Matcher.knnMatch(descriptors1, descriptors2, k=2)
    ...
    # for i,octave_keypoints in enumerate(keypoints1):
    #     for keypoint in octave_keypoints:
    #         x= keypoint['x']
    #         y=keypoint['y']
    #         cv2.circle(img1, ((2**(i))*x, (2**(i))*y), 5, (0, 255, 0), 2)  # Green circles with radius 5 and thickness 2
